...
export default {
  ...
  methods: {
    cancelBooking() {
      ...
      client.request(cancelBooking, variables)
        .then(() => {
          EventBus.$emit('show-info-notification-long', 'Your booking was cancelled. Paid amount will be refunded in your account within the next 48 hours.');
        })
        .catch((err) => {
