...
import AddTent from '../../components/AddTent.vue'
import EditTent from '../../components/EditTent.vue'
import CloseBooking from '../../components/CloseBookingsByDate.vue'
...
export default {
  ...
  methods: {
    ...
    openTentBooking(isCloseBooking, tentId) {
      ...
      client
        .request(closeTentBooking, variables)
        .then(data => {
          if (data.closeTentBooking.isAvailable === false) {
            EventBus.$emit(
              'show-info-notification-short',
              'Booking Closed for this Tent'
            )
          } else {
            EventBus.$emit(
              'show-info-notification-short',
              'Booking Open for this Tent'
            )
          }
          this.getAllTents()
        })
    ...
    campBookingStatus(campAvailable, campId) {
      ...
      client
        .request(campBooking, variables)
        .then(data => {
          if (data.campAvailability.isAvailable === false) {
            EventBus.$emit(
              'show-info-notification-short',
              'Booking Closed for this Camp'
            )
          } else {
            EventBus.$emit(
              'show-info-notification-short',
              'Booking Open for this Camp'
            )
          }
          this.getCampStatus()
        })
