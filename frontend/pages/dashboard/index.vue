      ...
      v-layout(row wrap)
        v-flex(xs12)
          v-container
            v-layout(column)
              h3.font-weight-black.text-uppercase Active bookings
              v-card(v-for="booking in adminBookings" :key="booking").py-5
                v-layout(row wrap)
                  v-flex(md2).pl-5
                    v-flex
                      h5.headline.font-weight-medium {{booking.user.name}}
...
export default {
  ...
  methods: {
    ...
    countBookings(campId) {
      ...
      client
        .request(countCampActiveBooking, variables)
        .then(data => {
          this.activeCampBooking =
            data.countCampActiveBookings.countActiveBooking
        })
        .catch(err => {
          EventBus.$emit(
            'show-error-notification-short',
