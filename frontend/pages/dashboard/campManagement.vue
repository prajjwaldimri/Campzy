...
import EditCamp from '../../components/EditCamp.vue'
...
export default {
  ...
  data() {
    return {
      headers: [
        ...
        { text: 'Camp Owner', value: 'ownerId.name' },
  ...
  methods: {
    ...
    getAllCamps(pageNumber) {
      ...
      client
        .request(getCampsQuery, variables)
        .then(data => {
    ...
    deleteCamp() {
      if (this.deleteCampName === this.deleteCampNameConfirmation) {
        ...
        client
          .request(deleteCampQuery, variables)
          .then(() => {
